...
## 输出要求
1. 分节：## 数据概览、## 付费产品分析、## 免费产品分析、## 异常与风险、## 7日预测解读、## 结论与建议
...
2. **Tab 切换**：「付费产品」「免费产品」，JavaScript 切换 panel。
3. 付费 Tab：新增、新增付费、充值、ARPPU、7日预测（Segments paid Forecasts）。
4. 免费 Tab：DAU、7日留存、新增、7日预测（Segments free Forecasts）。
5. 预测表须展示 MethodDetail、SampleDays；carry_forward 须注明样本不足。
6. 禁止编造数据。
7. 显眼展示 AI 标识：%s；<meta name="generator" content="%s"/>
8. 只输出 HTML，不要 markdown 或解释文字。`,
		skillDoc.ID, skillDoc.SHA256, skillDoc.Body, badgeText, badgeText)
}
